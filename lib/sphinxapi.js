...
const debug = {
  debug: require('debug')('sphinxapi:debug'),
  client: require('debug')('sphinxapi:client'),
  server: require('debug')('sphinxapi:server')
};
...
function SphinxClient() {
  ...
  this._socket = null
  // how much records to seek from result-set start
  this._offset = 0
  this._limit = 20
  this._queryMatchMode = SphinxClient.SPH_MATCH_ALL
  ...
  // ranking expression for SPH_RANK_EXPR
  this._rankexpr = ''
...
* @api private
*/
SphinxClient.prototype._SendRequest = function (clientVersion, request, fn) {
  var self = this
  var client = false
  if (self._unixSocket) {
    client = net.connect(self._unixSocket)
    debug('Connecting to ' + self._unixSocket)
  ...
  client.once('data', function (chunk) {
      ...
      if (!Array.isArray(response)) {
        ...
        debug.client('received version', clientVersion);
      }
      var content, state, version, length;
      client.on('data', function (chunk) {
          if (content === null || content === undefined) {
            debug.client('received the response');
            response = unpack('>2HL', chunk)
            debug.server('>2HL', response)
            state   = response[0]
            version = response[1]
            length  = response[2]
            content = chunk.slice(8)
            debug.client('processing the response #1', state, version, length)
          }
          else {
            debug.client('received following the response ')
            content = ConcatBuffer(content, chunk)
          }
          if (content.length >= length) {
            var err = null
            debug.client('processing the response #2', state, version);

            if (state == SphinxClient.SEARCHD_WARNING) {
              var wend = 4 + unpack('>L', content)
              self._warning = content.slice(4, wend);
            ...
            if (version < clientVersion) {
              self._warning = util.format('searchd command v.%d.%d older '
              + 'than client\'s v.%d.%d, some options might not work',
                version >> 8, version & 0xff, clientVersion >> 8, clientVersion & 0xff)
              // TODO do something with the warning !!!
            }
      ...
      debug.client('sending a request', request.toString());
...
SphinxClient.prototype.SetMatchMode = function (mode) {
  var self = this
  var modes = [
    SphinxClient.SPH_MATCH_ALL,
    SphinxClient.SPH_MATCH_ANY,
    SphinxClient.SPH_MATCH_PHRASE,
    SphinxClient.SPH_MATCH_BOOLEAN,
    SphinxClient.SPH_MATCH_EXTENDED,
    SphinxClient.SPH_MATCH_FULLSCAN,
    SphinxClient.SPH_MATCH_EXTENDED2
  ]
  assert(modes.some(function (x) { return (x === mode) }))
  self._queryMatchMode = mode
};
...
SphinxClient.prototype.SetSortMode = function (mode, clause) {
  var self = this
  var modes = [
    SphinxClient.SPH_SORT_RELEVANCE,
    SphinxClient.SPH_SORT_ATTR_DESC,
    SphinxClient.SPH_SORT_ATTR_ASC,
    SphinxClient.SPH_SORT_TIME_SEGMENTS,
    SphinxClient.SPH_SORT_EXTENDED,
    SphinxClient.SPH_SORT_EXPR
  ]
  if (clause === undefined) {
    clause = ''
  }
...
SphinxClient.prototype.SetIDRange = function (minid, maxid) {
  ...
  assert(minid <= maxid)
  self.minMatchID = minid
  self.maxMatchID = maxid
};
...
SphinxClient.prototype.SetGroupBy = function (attribute, func, groupsort ) {
  ...
  var funcs = [
    SphinxClient.SPH_GROUPBY_DAY,
    SphinxClient.SPH_GROUPBY_WEEK,
    SphinxClient.SPH_GROUPBY_MONTH,
    SphinxClient.SPH_GROUPBY_YEAR,
    SphinxClient.SPH_GROUPBY_ATTR,
    SphinxClient.SPH_GROUPBY_ATTRPAIR
  ]
...
SphinxClient.prototype.AddQuery = function (query, index, comment) {
  ...
  var req = []
  req.push(pack('>LLLL', [self._offset, self._limit, self._queryMatchMode, self._ranker]))
  ...
  forEach(self._weights, function (item) {
  ...
  //    req.push(pack('>Q',  [self.minMatchID]))
  req.push(packUInt64(self.minMatchID))
  //    req.push(pack('>Q', [self.maxMatchID]))
  req.push(packUInt64(self.maxMatchID))

  // filters
  req.push(pack('>L', [len(self._filters)]))
  forEach(self._filters, function (f) {
  ...
  forEach(self._overrides, function (v) {
  ...
  debug.debug('New Request Added', req.toString());
...
SphinxClient.prototype.RunQueries = function (fn) {
  ...
  debug.client('>HHLLL', [
    SphinxClient.SEARCHD_COMMAND_SEARCH,
    SphinxClient.VER_COMMAND_SEARCH,
    length,
    0,
    nreqs]);
  var request = ConcatBuffer(pack('>HHLLL', [
    SphinxClient.SEARCHD_COMMAND_SEARCH,
    SphinxClient.VER_COMMAND_SEARCH,
    length,
    0,
    self._reqs.length]), req)
...
SphinxClient.prototype.BuildExcerpts = function (docs, index, words, opts, cb) {
  ...
  opts.beforeMatch = typeof opts.beforeMatch !== 'undefined'
    ? opts.beforeMatch
    : "<b>";
  opts.afterMatch = typeof opts.afterMatch !== 'undefined'
    ? opts.afterMatch
    : "</b>";
  opts.chunkSeparator = typeof opts.chunkSeparator !== 'undefined'
    ? opts.chunkSeparator
    : " ... ";
  opts.limit = typeof opts.limit !== 'undefined'
    ? opts.limit
    : 256;
  opts.limitPassages = typeof opts.limitPassages !== 'undefined'
    ? opts.limitPassages
    : 0;
  opts.limitWords = typeof opts.limitWords !== 'undefined'
    ? opts.limitWords
    : 0;
  opts.around = typeof opts.around !== 'undefined'
    ? opts.around
    : 5;
  opts.exactPhrase = typeof opts.exactPhrase !== 'undefined'
    ? opts.exactPhrase
    : false;
  opts.singlePassage  = typeof opts.singlePassage !== 'undefined'
    ? opts.singlePassage
    : false;
  opts.useBoundaries  = typeof opts.useBoundaries !== 'undefined'
    ? opts.useBoundaries
    : false;
  opts.weightOrder = typeof opts.weightOrder !== 'undefined'
    ? opts.weightOrder
    : false;
  opts.queryMode  = typeof opts.queryMode !== 'undefined'
    ? opts.queryMode
    : false;
  opts.forceAllWords = typeof opts.forceAllWords !== 'undefined'
    ? opts.forceAllWords
    : false;
  opts.startPassageId = typeof opts.startPassageId !== 'undefined'
    ? opts.startPassageId
    : 1;
  opts.loadFiles  = typeof opts.loadFiles !== 'undefined'
    ? opts.loadFiles
    : false;
  opts.htmlStripMode = typeof opts.htmlStripMode !== 'undefined'
    ? opts.htmlStripMode
    : "index";
  opts.allowEmpty = typeof opts.allowEmpty !== 'undefined'
    ? opts.allowEmpty
    : false;
  opts.passageBoundary = typeof opts.passageBoundary !== 'undefined'
    ? opts.passageBoundary
    : "none";
  opts.emitZones  = typeof opts.emitZones !== 'undefined'
    ? opts.emitZones
    : false;
  opts.loadFilesScattered = typeof opts.loadFilesScattered !== 'undefined'
    ? opts.loadFilesScattered
    : false;

  var flags = 1; // remove spaces
  if (opts.exactPhrase) flags = flags | 2;
  if (opts.singlePassage) flags = flags | 4;
  if (opts.useBoundaries) flags = flags | 8;
  if (opts.weightOrder) flags = flags | 16;
  if (opts.queryMode) flags = flags | 32;
  if (opts.forceAllWords) flags = flags | 64;
  if (opts.loadFiles) flags = flags | 128;
  if (opts.allowEmpty) flags = flags | 256;
  if (opts.emitZones) flags = flags | 512;
  if (opts.loadFilesScattered) flags = flags | 1024;
  ...
  req.push(pack('>L', [len(opts.beforeMatch)]), opts.beforeMatch);
  req.push(pack('>L', [len(opts.afterMatch)]), opts.afterMatch);
  req.push(pack('>L', [len(opts.chunkSeparator)]), opts.chunkSeparator);
  req.push(pack('>LL', [opts.limit, opts.around]));
  req.push(pack('>LLL', [opts.limitPassages, opts.limitWords, opts.startPassageId]));
  req.push(pack('>L', [len(opts.htmlStripMode)]), opts.htmlStripMode);
  req.push(pack('>L', [len(opts.passageBoundary)]), opts.passageBoundary);
  ...
  var request = ConcatBuffer(pack('>HHL', [
    SphinxClient.SEARCHD_COMMAND_EXCERPT,
    SphinxClient.VER_COMMAND_EXCERPT,
    length]), reqData);
...
SphinxClient.prototype.BuildKeywords = function (query, index, hits, cb ) {
  ...
  var request = ConcatBuffer(pack('>2HL', [
    SphinxClient.SEARCHD_COMMAND_KEYWORDS,
    SphinxClient.VER_COMMAND_KEYWORDS,
    length]), reqData);
...
SphinxClient.prototype.Status = function (fn) {
  var self = this
  debug.client('>2HLL', [
    SphinxClient.SEARCHD_COMMAND_STATUS,
    SphinxClient.VER_COMMAND_STATUS,
    4,
    1]);
  var request = pack( '>2HLL', [
    SphinxClient.SEARCHD_COMMAND_STATUS,
    SphinxClient.VER_COMMAND_STATUS,
    4,
    1])
...
SphinxClient.prototype.EscapeString = function () {
};
SphinxClient.prototype.FlushAttributes = function (fn) {
  var self = this
  var request = pack ( '>hhI', [
    SphinxClient.SEARCHD_COMMAND_FLUSHATTRS,
    SphinxClient.VER_COMMAND_FLUSHATTRS,
    0] )
